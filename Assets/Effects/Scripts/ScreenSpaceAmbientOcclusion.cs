...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class ScreenSpaceAmbientOcclusion : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            CreateMaterials();
            if (!_SSAOMaterial || _SSAOMaterial.passCount != 5)
            {
                _supported = false;
                enabled = false;
                return;
        ...
        {
            if (!_SSAOMaterial && m_SSAOShader.isSupported)
            {
                _SSAOMaterial = CreateMaterial(m_SSAOShader);
                _SSAOMaterial.SetTexture("_RandomTexture", m_RandomTexture);
            }
        }
        ...
        {
            if (!_supported || !m_SSAOShader.isSupported)
            {
                enabled = false;
                return;
            }
            ...
            _SSAOMaterial.SetVector("_NoiseScale", new Vector3((float)rtAO.width / noiseWidth, (float)rtAO.height / noiseHeight, 0.0f));
            ...
            Graphics.Blit(doBlur ? null : source, rtAO, _SSAOMaterial, (int)m_SampleCount);
