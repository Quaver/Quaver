...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class ColorCorrectionLookup : PostEffectsBase
    {
        ...
        private void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            _material.SetFloat("_Scale", (lutSize - 1) / (1.0f * lutSize));
            _material.SetFloat("_Offset", 1.0f / (2.0f * lutSize));
            _material.SetTexture("_ClutTex", converted3DLut);

            Graphics.Blit(source, destination, _material, QualitySettings.activeColorSpace == ColorSpace.Linear ? 1 : 0);
        }
    }
}
