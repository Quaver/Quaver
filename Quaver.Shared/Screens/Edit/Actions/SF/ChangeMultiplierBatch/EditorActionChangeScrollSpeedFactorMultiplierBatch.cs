...
namespace Quaver.Shared.Screens.Edit.Actions.SF.ChangeMultiplierBatch
{
    ...
    public class EditorActionChangeScrollSpeedFactorMultiplierBatch : IEditorAction
    {
            ...
            float multiplier)
        {
            ...
            ScrollSpeedFactors.ForEach(x => OriginalMultipliers.Add(x.Factor));
        }

        [MoonSharpVisible(false)]
        public void Perform()
        {
            ScrollSpeedFactors.ForEach(x => x.Factor = Multiplier);
        ...
        public void Undo()
        {
            for (var i = 0; i < ScrollSpeedFactors.Count; i++)
                ScrollSpeedFactors[i].Factor = OriginalMultipliers[i];
