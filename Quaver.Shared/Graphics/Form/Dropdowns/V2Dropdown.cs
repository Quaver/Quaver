...
namespace Quaver.Shared.Graphics.Form.Dropdowns
{
    ...
    public sealed class DropdownOption<T> : DropdownEntry<T>
    {
        ...
        public DropdownOption(T value, string label, Color? textColor = null, string shortLabel = null, TextureRegion? icon = null)
        {
            Value = value;
            TextColor = textColor;
        ...
        public DropdownOption(T value, string label, Texture2D icon, string shortLabel = null)
            : this(value, label, null, shortLabel, icon == null ? (TextureRegion?) null : new TextureRegion(icon, icon.Bounds))
        {
        }
    }
    ...
    /// <summary>
    ///     Shared V2 dropdown menu. Selectors keep a trigger in their owner's layout; action menus
    ///     use a triggerless anchor. Both draw their options in the caller-provided overlay host.
    /// </summary>
    public class V2Dropdown<T> : V2DropdownBase
    {
        ...
        private bool IgnoreOpeningClick { get; set; }
        ...
        protected V2Dropdown(float width, IReadOnlyList<DropdownEntry<T>> entries,
            WobbleFontStore font, SkinV2DropdownConfig config, Container overlayHost)
            : this(width, entries, font, config, overlayHost, DropdownSelectionMode.Action,
                null, null)
        {
        }
            ...
            IEnumerable<T> selectedValues)
        {
            ...
            Size = new ScalableVector2(width, selectionMode == DropdownSelectionMode.Action ? 0 : config.Height);
            if (selectionMode != DropdownSelectionMode.Action)
            {
        ...
        {
            if (Opened && !DialogManager.IsInputAllowed(OverlayHost))
            ...
            var outsideClick = MouseManager.IsUniqueClick(MouseButton.Left) ||
                               SelectionMode == DropdownSelectionMode.Action &&
                               MouseManager.IsUniqueClick(MouseButton.Right);
            if (outsideClick &&
                (Trigger == null || !Contains(Trigger.ScreenRectangle, MouseManager.CurrentState.Position)) &&
        ...
        public void Open()
        {
            if (Opened || !DialogManager.IsInputAllowed(OverlayHost) || ExternalAlpha <= 0.001f)
                return;
            ...
            IgnoreOpeningClick = SelectionMode == DropdownSelectionMode.Action;
            CreateMenu();
            if (Trigger != null)
            {
        ...
        private void CreateMenu()
        {
            ...
            MenuDivider = new Sprite
            {
                ...
                Alpha = 0,
                Visible = SelectionMode != DropdownSelectionMode.Action
            };
        ...
        private bool IsMenuScrollable()
        {
            ...
            var targetViewportHeight = Math.Max(MinimumMenuHeight,
                TargetMenuHeight - (MenuDivider.Visible ? Config.DividerThickness : 0) -
                Config.MenuPadding * 2);
        ...
        private void RefreshMenuLayout(bool applyMenuHeight = true)
        {
            ...
            var anchor = Trigger?.ScreenRectangle ?? ScreenRectangle;
            var availableBelow = WindowManager.Height - anchor.Bottom - Config.MenuGap;
            var availableAbove = anchor.Top - Config.MenuGap;
            ...
            var x = MathHelper.Clamp(anchor.Left, 0,
                Math.Max(0, WindowManager.Width - Width));
            var y = OpensDown
                ? anchor.Bottom + Config.MenuGap
                : anchor.Top - Config.MenuGap - currentHeight;
            ...
            var dividerSpace = MenuDivider.Visible ? Config.DividerThickness : 0;
            var menuPadding = Config.MenuPadding;
            var viewportHeight = Math.Max(MinimumMenuHeight,
                currentHeight - dividerSpace - menuPadding * 2);
            var viewportPosition = new ScalableVector2(0,
                OpensDown ? dividerSpace + menuPadding : menuPadding);
        ...
        private float CalculateTargetMenuHeight(float contentHeight, int optionCount)
        {
            ...
            if (visibleItems >= optionCount)
                return (float) Math.Ceiling(contentHeight +
                                            Config.MenuPadding * 2 +
                                            (SelectionMode == DropdownSelectionMode.Action ? 0 : Config.DividerThickness));
            ...
            return (float) Math.Ceiling(visibleHeight +
                                        Config.MenuPadding * 2 +
                                        (SelectionMode == DropdownSelectionMode.Action ? 0 : Config.DividerThickness));
        }
        ...
        private sealed class V2DropdownOptionButton : RoundedButton
        {
            ...
            public void SetSelected(bool selected)
            {
                ...
                if (MarqueeLabel != null)
                    MarqueeLabel.TextSprite.Tint = Option.TextColor ?? SkinV2Color.Parse(
