    ...
    internal class EditorInputManager
    {
        ...
        private void HandleSeeking()
        {
            ...
            if (InputHelper.IsUniqueKeyPress(Keys.Left) || scrollDiff > 0)
                GameBase.AudioEngine.SeekToNearestSnap(Screen.Map, SeekDirection.Backward, Screen.CurrentBeatSnap.Value);
            
            if (InputHelper.IsUniqueKeyPress(Keys.Right) || scrollDiff < 0)
                GameBase.AudioEngine.SeekToNearestSnap(Screen.Map, SeekDirection.Forward, Screen.CurrentBeatSnap.Value);
        }
    }
}