    ...
    internal class DiscordController
    {
        ...
        public static void ChangeDiscordPresence(string details, string state, double timeLeft = 0)
        {
            ...
            try
            {
                ...
                GameBase.DiscordController.presence.largeImageText = ConfigManager.Username;

                // Set username and rank if connected to Flamingo.
                if (Flamingo.Connected && Flamingo.Self != null && GameBase.SelectedMap != null)
                {
                    if (Flamingo.Self.UserStats[GameBase.SelectedMap.Mode].Rank != 0)
                        GameBase.DiscordController.presence.largeImageText = $"{Flamingo.Self.Username}: Rank #{Flamingo.Self.UserStats[GameBase.SelectedMap.Mode].Rank}";
                }
