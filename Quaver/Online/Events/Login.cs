        ...
        public static void OnLoginReplyEvent(object sender, LoginReplyEventArgs e)
        {
            switch (e.LoginErrorCode)
            {
                case LoginErrorCodes.Success:
                    Logger.LogSuccess($"Successfully logged in as {Flamingo.Self.Username} <{Flamingo.Self.SteamId}> ({Flamingo.Self.UserId}) - ${Flamingo.Self.Country}", LogType.Network);
                    ...
                    ConfigManager.Username = Flamingo.Self.Username;
