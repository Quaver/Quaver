...
namespace Quaver.Online
{
    public static class SteamManager
    {
        ...
        public static void Initialize()
        {
            ...
            if (!IsInitialized)
            {
                Logger.LogError($"SteamAPI.Init() call has failed! Steam is not initialized", LogType.Network);
                throw new Exception();
            }

            if (!Packsize.Test())
            {
                Logger.LogError($"The incorrect Steamworks.NET assembly was loaded for this platform!", LogType.Runtime);
                throw new Exception();
            }

            if (!DllCheck.Test())
            {
                Logger.LogError($"The wrong dlls were loaded for this platform!", LogType.Runtime);
                throw new Exception();
            }

            Logger.LogSuccess($"Successfully initialized and logged into Steam as : {SteamFriends.GetPersonaName()} " +
